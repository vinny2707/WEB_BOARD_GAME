...
const BoardGame = () => {
  ...
  // Timer effect
  useEffect(() => {
    if (mode === MODES.PLAYING && gameState?.status === 'playing') {
    ...
    try {
      if (gameApiId) {
        ...
        if (detailRes.success && detailRes.data?.settings?.boardSize) {
          setSizeOptions(detailRes.data.settings.boardSize.options || []);
          ...
          setSizeOptions([{ label: "3x3", value: 3 }]);
  ...
  }, [gameModule, activeGameKey, sizeOptions, selectedSizeIndex, isAuthenticated, apiGameId, startSession]);
  ...
  const handleLeft = useCallback(() => {
    if (mode === MODES.GAME_SELECT) {
      ...
      if (gameState.status === 'playing' && gameState.currentPlayer === 'X' && !gameState.isAIThinking) {
        const newCell = gameModule.getCellFromNav(gameState, 'left');
  ...
  const handleRight = useCallback(() => {
    if (mode === MODES.GAME_SELECT) {
      ...
      if (gameState.status === 'playing' && gameState.currentPlayer === 'X' && !gameState.isAIThinking) {
        const newCell = gameModule.getCellFromNav(gameState, 'right');
        setGameState(prev => ({ ...prev, selectedCell: newCell }));
      }
    }
  }, [mode, sizeOptions.length, gameModule, gameState]);

  const handleUp = useCallback(() => {
    if (mode === MODES.PLAYING && gameModule && gameState) {
      if (gameState.status === 'playing' && gameState.currentPlayer === 'X' && !gameState.isAIThinking) {
        const newCell = gameModule.getCellFromNav(gameState, 'up');
        setGameState(prev => ({ ...prev, selectedCell: newCell }));
      }
    }
  }, [mode, gameModule, gameState]);
  ...
  const handleDown = useCallback(() => {
    if (mode === MODES.PLAYING && gameModule && gameState) {
      if (gameState.status === 'playing' && gameState.currentPlayer === 'X' && !gameState.isAIThinking) {
        const newCell = gameModule.getCellFromNav(gameState, 'down');
        setGameState(prev => ({ ...prev, selectedCell: newCell }));
      }
    }
  }, [mode, gameModule, gameState]);
  ...
  }, [mode, gameState, enterSettings, startGame, handlePlayerMove, resetGame]);
  ...
  }, [mode, currentGamePattern, sizeOptions, selectedSizeIndex, gameModule, gameState]);

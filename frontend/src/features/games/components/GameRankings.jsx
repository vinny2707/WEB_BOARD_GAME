 ...
 */
const GameRankings = ({ gameId, themeColor = 'emerald', limit = 8, showCountdown = true }) => {
    ...
    const fetchRankings = useCallback(async () => {
        ...
        try {
            ...
            const response = await getGameRankings(gameId, { limit, scope: 'global' });
    ...
    const getCurrentUserName = () => {
        try {
            ...
            if (userStr) {
                ...
                return user.username || user.full_name || 'You';
            }
        } catch (e) {
                ...
                <>
                    <div className="flex flex-col gap-2">
                        {rankings.map((player) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className={`flex-1 text-sm font-medium truncate ${isMe ? colors.text : 'text-foreground'}`}>
                                        {isMe ? getCurrentUserName() : (player.user?.username || player.user?.full_name || 'Unknown')}
                                        {isMe && <span className="ml-1 text-xs opacity-75">(Bạn)</span>}
                                    </span>
                                    <span className={`text-sm font-semibold ${isMe ? colors.text : 'text-muted-foreground'}`}>
                        ...
                        {myRanking && !isMyRankInList() && (
                            <div className={`flex items-center gap-3 px-3 py-2 rounded-lg bg-gradient-to-r ${colors.bg} to-transparent border ${colors.border} mt-2`}>
                                <div className="w-7 text-center">
                                    <span className={`text-sm font-semibold ${colors.text}`}>{myRanking.rank}.</span>
                                </div>
                                <div className="text-xl">🎮</div>
